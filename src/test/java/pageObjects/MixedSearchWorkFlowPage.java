...
public class MixedSearchWorkFlowPage extends BasePage{
	Random ran = new Random();
	Select s;

	public String rn = "MixedSearchAutomationTestReport"+GenerateRandomStrings.generateNumber();
	public MixedSearchWorkFlowPage(WebDriver driver) {
		super(driver);
	}
...
//	List<WebElement> tgBtns = driver.findElements(toggleButton);
//	List<WebElement> cdd = driver.findElements(criteriaDropdowns);
//	List<WebElement> minTF = driver.findElements(rangeMinField);
//	List<WebElement> maxTF = driver.findElements(rangeMaxField);
	...
	public void SelectProperties() throws InterruptedException {
		  List<WebElement> rows = ww.until(ExpectedConditions.numberOfElementsToBeMoreThan(propRows, 0));
	        ...
	        for (int i = 0; i < rows.size(); i++) {
	            String selected = criteriaOptions.get(ran.nextInt(criteriaOptions.size()));
	...
	public void verifyReportNameInReport() {
    	...
    	String ExpReportNametxt = MixedSearchWorkFlowPage.this.rn;
