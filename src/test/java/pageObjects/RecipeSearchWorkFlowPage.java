...
public class RecipeSearchWorkFlowPage extends BasePage{
	
	Random ran = new Random();
	Select s;
	public String rn = "RecipeSearchAutomationTestReport"+GenerateRandomStrings.generateNumber();

	public RecipeSearchWorkFlowPage(WebDriver driver) {
		super(driver);
	}
	
	By ReportName = By.xpath("//input[@id='prediction-profile-name']");
	By datasetDDBy = By.xpath("//span[text()='Select dataset']");
	By dataset = By.xpath("//li[normalize-space()='Cycoloy FR Pmml']");
	...
	By addButton = By.xpath("//i[@class='fas fa-plus text-secondary']");
	List<String> criteriaOptions = List.of("Greater Than (>)", "Less Than (<)", "Range");
    By GenerateButton = By.xpath("//a[@href='#finish']");
    By reportNameInRsreport = By.xpath("//h5[contains(@class,'mr-2 text-uppercase font-weight-normal')]");
    
    public void SearchDetailsTab(String name) throws InterruptedException {
		WebElement rr = driver.findElement(ReportName);
		rr.clear();
		rr.sendKeys(name);
		System.out.println("Report Name Entered: " + name);
	}
    
    public void clickOnDataset() throws InterruptedException {
    	WebElement dd = driver.findElement(datasetDDBy);
		dd.click();
		
		//WebElement typeOption = driver.findElement(dataset);
		WebElement typeOption=ww.until(ExpectedConditions.elementToBeClickable(dataset));
		typeOption.click();
		//System.out.println("Dataset Selected is : "+ typeOption.getText());
		Thread.sleep(5000);
    }
    ...
    public void clickOnNextButton() throws InterruptedException {
    	 ...
    	 WebElement button = ww.until(ExpectedConditions.presenceOfElementLocated(NextButton));
    	    ...
    	    // Sometimes JS click is more reliable than WebElement.click()
    	    ((JavascriptExecutor) driver).executeScript("arguments[0].click();", button);
    	
    }
    
	public void AddIngredients() throws InterruptedException {
        ...
        for (int i = 0; i < countToClick; i++) {
           ...
           //  Thread.sleep(3000);
            ...
            String selectIngCri = criteriaOptions.get(ran.nextInt(criteriaOptions.size()));
    ...
    public void verifyReportNameInReport() {
		String ActReportNametxt = driver.findElement(reportNameInRsreport).getText();
    	String ExpReportNametxt = RecipeSearchWorkFlowPage.this.rn;

		Assert.assertEquals(ActReportNametxt.toLowerCase(), ExpReportNametxt.toLowerCase(), "Report name does not match in the Performance Prediction report.");
	}
